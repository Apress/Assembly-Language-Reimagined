;============================================================================
; environment.asm - demonstrates invokidng getenv, printf and strncpy 
...
; declaration: int printenv(const char* sDateTime);
...
%macro getSaveEnv	1				;===== getSaveEnv macro takes 1 arg =====
	...
	or		rax, rax				; did we get an invalid value (rax == 0)?
	cmovz	rsi, rcx				; 	if yes, strncpy "(null)"
...
printenv:							
	...
	push 	rdi						; save arg in auto variable (dateStr)
	...
	; we've used all 6 argument passing registers - PUSH remaining 9 args
	...
	add		rsp, NUM_PUSH*VAR_SIZE	; caller must remove items pushed
	xor		eax, eax				; return EXIT_SUCCESS = 0
...
section		.rodata
formatString	db LF,  "Environment Variables on %s",
				db TAB, "HOME     = %s",				LF
