...
; 12/07/2023		Linked with bubblesortStr, quicksortStr
; Linux x64
;
;============================================================================
ARRAY_SIZE		equ  256
ELEMENT_SIZE	equ	   8			; for QuicksortInt

%include "macro.inc"

global	main
extern	bubblesortInt					; external procedures
...
swapij:
	...
	mov		rcx, r8
	pop		rdi						; nTempj
	call	putElement				; rdi contains int from array[j]
...
main:
	...
	puts	[bsorted]               ; puts (put string) is like printf - writes single str followed by linefeed
	zero	rdi						; rsi = nLow
	mov		rsi, nRecords			; rdi = nHigh
	call	bubblesortInt			; integer version of quicksort(nLow, nHigh)
...
prtInsTotal	db	TAB, LF, "%ld elements were inserted.", LF, EOL
prtInsItem	db	TAB, "Inserted element %ld = %ld", LF, EOL
