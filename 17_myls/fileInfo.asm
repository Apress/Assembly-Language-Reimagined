...
; Sat Nov  2 12:04:59 PM EDT 2024
; Linux x86_64
;============================================================================
%include	"macro.inc"
...
readFileInfo:
	...
	lea		rsi, [roperm]			; copy read-only perm to perm
	lea		rdi, [perm]
	strcpy

	lea		rsi, [roperm]			; init permissions
	lea		rdi, [perm]
	mov		rcx, ROPERM_LEN
	rep		movsb
...
nFileTime	resq	 1
nMode		resd	 1
linkName	resb	STD_STR_LEN
fileName	resb	STD_STR_LEN
sNameLink	resb	STD_STR_LEN
...
pw		istruc	pw_struct			; password structure
		iend

gr		istruc	gr_struct			; group structure
		iend
